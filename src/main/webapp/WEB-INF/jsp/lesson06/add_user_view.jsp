...
<body>
	...
	${document).ready(function()
		// (1) jquesy의 submit 기능 이용하기	
		/* $("form").on('submit', function(e) {
			e.priventDefault();	// submit 되는 것을 막는다.
			//alert("폼 서브밋");
			...
			if (yyyymmdd.length < 1){
				alert("생년월일을 입력해주세요.");
				return false;	// submit일때 return false 라고 해주기
			}
				// 여기까지 도달하면 submit
				alert("submit 수행");
		}); */
		
		// (2) jquery 의 AJAX 통신을 이용하기
		// AJAX를 사용할 때는 절대로 submit 버튼 사용 하지말고 버튼으로 사용하기
		...
		$('#addBtn').on('click', function(e) {
			...
			if (yyyymmdd.length < 1){
				alert("생년월일을 입력해주세요.");
				return;	// submit일때 return false 라고 해주기
			}
			
			if (isNaN(yyyymmdd)) {	// 모두 숫자가 아닐 때 true가 나온다.
				alert("숫자만 입력 해주세요");
				return;
			}
			}); 
